...
from corpochain.rpc.util import wrap_http_handler
from corpochain.server.outbound_message import NodeType
from corpochain.server.server import CorpochainServer, ssl_context_for_client, ssl_context_for_server
...
from corpochain.util.config import str2bool
from corpochain.util.ints import uint16
from corpochain.util.json_util import dict_to_json_str
from corpochain.util.network import WebServer, get_host_addr
from corpochain.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> CorpochainServer:
        ...
        # a property so as to be read only which allows CorpochainServer to satisfy
        # Optional[CorpochainServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        corpochain.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: CorpochainServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
