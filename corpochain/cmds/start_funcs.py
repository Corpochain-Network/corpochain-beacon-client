...
from corpochain.cmds.passphrase_funcs import get_current_passphrase
from corpochain.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from corpochain.util.errors import KeychainMaxUnlockAttempts
from corpochain.util.keychain import Keychain
from corpochain.util.service_groups import services_for_groups
...
async def async_start(root_path: Path, config: Dict[str, Any], group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the corpochain daemon")
        return None
