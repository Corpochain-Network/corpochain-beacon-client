...
from corpochain.cmds.cmds_util import NODE_TYPES, get_any_service_client
...
async def add_node_connection(rpc_client: RpcClient, add_connection: str) -> None:
    if ":" not in add_connection:
        print("Enter a valid IP and port in the following format: 10.5.4.3:9000")
    else:
        ip, port = (
...
async def print_connections(rpc_client: RpcClient, trusted_peers: Dict[str, Any]) -> None:
    ...
    from corpochain.server.outbound_message import NodeType
    from corpochain.util.network import is_trusted_inner
