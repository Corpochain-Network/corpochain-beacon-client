...
from corpochain.cmds.configure import configure
from corpochain.ssl.create_ssl import create_all_ssl
from corpochain.util.config import (
    create_default_corpochain_config,
    initial_config_file,
    load_config,
...
from corpochain.util.derive_keys import master_sk_to_pool_sk
...
def init(
    ...
    if create_certs is not None:
        ...
        return corpochain_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)

    return None


def corpochain_version_number() -> Tuple[str, str, str, str]:
...
def corpochain_full_version_str() -> str:
    major, minor, patch, dev = corpochain_version_number()
...
def corpochain_init(
    ...
    *,
    ...
    """
    corpochain_root = os.environ.get("CORPOCHAIN_ROOT", None)
    if corpochain_root is not None:
        print(f"CORPOCHAIN_ROOT is set to {corpochain_root}")
    ...
    create_default_corpochain_config(root_path)
    if testnet:
        configure(
            root_path,
            set_farmer_peer="",
