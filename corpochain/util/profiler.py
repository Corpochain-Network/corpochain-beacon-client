...
# the output will be printed to your corpochain root path, e.g. ~/.corpochain/beacon/profile/
# to analyze the profile, run:

#   python corpochain/utils/profiler.py ~/.corpochain/beacon/profile | less -r

# this will print CPU usage of the Corpochain Beacon Client main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python corpochain/utils/profiler.py ~/.corpochain/beacon/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "corpochain-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
