...
from corpochain.util.default_root import DEFAULT_KEYS_ROOT_PATH
from corpochain.util.errors import KeychainFingerprintNotFound, KeychainLabelExists, KeychainLabelInvalid
from corpochain.util.lock import Lockfile
from corpochain.util.streamable import convert_byte_type
...
def get_symmetric_key(salt: bytes) -> bytes:
    from corpochain.cmds.passphrase_funcs import obtain_current_passphrase
...
class FileKeyring(FileSystemEventHandler):  # type: ignore[misc] # Class cannot subclass "" (has type "Any")
    ...
    def write_keyring(self, fresh_salt: bool = False) -> None:
        from corpochain.cmds.passphrase_funcs import obtain_current_passphrase
        from corpochain.util.keyring_wrapper import KeyringWrapper
