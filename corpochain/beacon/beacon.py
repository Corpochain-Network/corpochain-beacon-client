...
from corpochain.consensus.block_creation import unfinished_block_to_full_block
from corpochain.consensus.block_record import BlockRecord
from corpochain.consensus.blockchain import Blockchain, ReceiveBlockResult, StateChangeSummary
...
from corpochain.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from corpochain.consensus.multiprocess_validation import PreValidationResult
from corpochain.consensus.pot_iterations import calculate_sp_iters
from corpochain.beacon.block_store import BlockStore
from corpochain.beacon.beacon_api import BeaconAPI
from corpochain.beacon.beacon_store import BeaconStore, BeaconStorePeakResult
from corpochain.beacon.lock_queue import LockClient, LockQueue
from corpochain.beacon.signage_point import SignagePoint
from corpochain.beacon.sync_store import SyncStore
from corpochain.beacon.weight_proof import WeightProofHandler
from corpochain.protocols import farmer_protocol, beacon_protocol, timelord_protocol
from corpochain.protocols.beacon_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
...
from corpochain.server.node_discovery import BeaconPeers
from corpochain.server.outbound_message import Message, NodeType, make_msg
from corpochain.server.peer_store_resolver import PeerStoreResolver
...
from corpochain.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from corpochain.util.check_fork_next_block import check_fork_next_block
from corpochain.util.config import process_config_start_method, lock_and_load_config, save_config
from corpochain.util.db_synchronous import db_synchronous_on
from corpochain.util.db_version import lookup_db_version
from corpochain.util.db_wrapper import DbWrapper, manage_connection
from corpochain.util.errors import ConsensusError, Err, ValidationError
from corpochain.util.ints import uint8, uint32, uint64, uint128
from corpochain.util.limited_semaphore import LimitedSemaphore
from corpochain.util.path import path_from_root
from corpochain.util.profiler import mem_profile_task, profile_task
from corpochain.util.safe_cancel_task import cancel_task_safe
from corpochain.beacon.execution_client import ExecutionClient
...
class Beacon:
    ...
    root_path: Path
    config: Dict[str, Any]
    _server: Optional[CorpochainServer]
    _shut_down: bool
    ...
    def set_server(self, server: CorpochainServer) -> None:
        self._server = server
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.mainnet.corpochain.pl")
        try:
    ...
    async def short_sync_batch(self, peer: WSCorpochainConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSCorpochainConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: beacon_protocol.NewPeak, peer: WSCorpochainConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSCorpochainConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSCorpochainConnection) -> None:
        """
    ...
    def on_disconnect(self, connection: WSCorpochainConnection) -> None:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[WSCorpochainConnection] = [
            ...
            weight_proof_peer: WSCorpochainConnection = random.choice(peers_with_peak)
            self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSCorpochainConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSCorpochainConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSCorpochainConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSCorpochainConnection, List[FullBlock]]]]
        ) -> None:
            advanced_peak: bool = False
            while True:
                res: Optional[Tuple[WSCorpochainConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSCorpochainConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSCorpochainConnection]:
    ...
    async def peak_post_processing(
        ...
        peer: Optional[WSCorpochainConnection],
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSCorpochainConnection] = None,
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSCorpochainConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSCorpochainConnection
    ...
    async def new_compact_vdf(self, request: beacon_protocol.NewCompactVDF, peer: WSCorpochainConnection) -> None:
    ...
    async def request_compact_vdf(self, request: beacon_protocol.RequestCompactVDF, peer: WSCorpochainConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: beacon_protocol.RespondCompactVDF, peer: WSCorpochainConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSCorpochainConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
