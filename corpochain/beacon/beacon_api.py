...
from corpochain.consensus.block_creation import create_unfinished_block
from corpochain.consensus.block_record import BlockRecord
from corpochain.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from corpochain.beacon.signage_point import SignagePoint
from corpochain.protocols import farmer_protocol, beacon_protocol, introducer_protocol, timelord_protocol
from corpochain.protocols.beacon_protocol import RejectBlock, RejectBlocks
...
from corpochain.util.limited_semaphore import LimitedSemaphoreFullError

if TYPE_CHECKING:
    from corpochain.beacon.beacon import Beacon
else:
    Beacon = object
...
class BeaconAPI:
    ...
    async def request_peers(
        self, _request: beacon_protocol.RequestPeers, peer: WSCorpochainConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSCorpochainConnection
    ) -> Optional[Message]:
    ...
    async def respond_signage_point(
        self, request: beacon_protocol.RespondSignagePoint, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        if self.beacon.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: beacon_protocol.RespondEndOfSubSlot, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        if self.beacon.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        if self.beacon.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSCorpochainConnection
    ) -> None:
        if self.beacon.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSCorpochainConnection
    ) -> Optional[Message]:
        if self.beacon.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: beacon_protocol.NewCompactVDF, peer: WSCorpochainConnection, request_bytes: bytes = b""
    ) -> None:
        if self.beacon.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: beacon_protocol.RespondCompactVDF, peer: WSCorpochainConnection) -> None:
        if self.beacon.sync_store.get_sync_mode():
            return None
