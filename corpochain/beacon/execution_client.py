...
from corpochain.types.blockchain_format.sized_bytes import bytes20, bytes32, bytes256
from corpochain.util.ints import uint64, uint256
from corpochain.types.blockchain_format.execution_payload import ExecutionPayloadV2, WithdrawalV1
from corpochain.util.byte_types import hexstr_to_bytes
from corpochain.consensus.block_rewards import create_withdrawals
...
class ExecutionClient:
    beacon: Beacon
    w3: Web3
    payload_cache: LRUCache[bytes32, str]
    syncing: bool
    ...
    def __init__(
        ...
        self.beacon = beacon
        self.w3 = None
        self.payload_cache = LRUCache(16)
        self.syncing = False
    ...
    async def forkchoice_update(
        ...
        if result.payloadId is not None:
            self.payload_cache.put(block.header_hash, result.payloadId)
            log.info(f"Payload building started: payloadId={result.payloadId}")
        elif synced:
    ...
    def get_payload(
        ...
        payload_id = self.payload_cache.get(prev_block.header_hash)
        if payload_id is None:
            raise RuntimeError(f"Payload ID for Bhash {prev_block.header_hash} not found in a cache")

        try:
            raw_payload = self.w3.engine.get_payload_v2(payload_id).executionPayload
        except:
            self.connected = False
            raise
        self.connected = True
        ...
        log.info(
            ...
            f"payload_id={payload_id}, Eheight={payload.blockNumber}, Ehash={payload.blockHash}"
        )
    ...
    def _ensure_web3_init(self) -> None:
        ...
        execution_endpoint = self.beacon.config.get("execution_endpoint", "http://127.0.0.1:9551")
