...
from corpochain.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_corpochain_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "corpochain_ca.crt")
    key = pkg_resources.resource_string(__name__, "corpochain_ca.key")
    return crt, key
...
def create_all_ssl(
    ...
    *,
    ...
    corpochain_ca_crt, corpochain_ca_key = get_corpochain_ca_crt_key()
    corpochain_ca_crt_path = ca_dir / "corpochain_ca.crt"
    corpochain_ca_key_path = ca_dir / "corpochain_ca.key"
    write_ssl_cert_and_key(corpochain_ca_crt_path, corpochain_ca_crt, corpochain_ca_key_path, corpochain_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./corpochain_ca.crt"), Path("./corpochain_ca.key"))
