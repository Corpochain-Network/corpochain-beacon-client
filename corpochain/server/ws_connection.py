...
from corpochain.protocols.protocol_state_machine import message_response_ok
from corpochain.protocols.protocol_timing import (
    API_EXCEPTION_BAN_SECONDS,
    INTERNAL_PROTOCOL_ERROR_BAN_SECONDS,
    CONSENSUS_ERROR_BAN_SECONDS
)
from corpochain.protocols.shared_protocol import Capability, Handshake
...
from corpochain.util.api_decorators import get_metadata
from corpochain.util.errors import Err, ProtocolError, ConsensusError
from corpochain.util.ints import uint8, uint16
from corpochain.util.log_exceptions import log_exceptions

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from corpochain.util.network import class_for_type, is_localhost
from corpochain.util.streamable import Streamable

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4

WebSocket = Union[WebSocketResponse, ClientWebSocketResponse]
ConnectionCallback = Callable[["WSCorpochainConnection"], Awaitable[None]]
...
class ConnectionClosedCallbackProtocol(Protocol):
    def __call__(
        self,
        connection: WSCorpochainConnection,
        ban_time: int,
