...
class PausableServer(BaseEventsServer):
    ...
    def _detach(self) -> None:
        ...
        if (
            ...
            self._corpochain_resume()

    def _corpochain_pause(self) -> None:
    ...
    def _corpochain_resume(self) -> None:
...
async def _corpochain_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class BpxSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _corpochain_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class BpxProactor(IocpProactor):
        ...
        async def _corpochain_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._corpochain_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _corpochain_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: BpxProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._corpochain_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class BpxProactorEventLoop(ProactorEventLoop):
...
class BpxPolicy(asyncio.DefaultEventLoopPolicy):
...
def set_corpochain_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(BpxPolicy())
