...
from corpochain.protocols.protocol_state_machine import message_requires_reply
from corpochain.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from corpochain.protocols.shared_protocol import protocol_version
from corpochain.server.introducer_peers import IntroducerPeers
from corpochain.server.outbound_message import Message, NodeType
from corpochain.server.ssl_context import private_ssl_paths, public_ssl_paths
from corpochain.server.ws_connection import ConnectionCallback, WSCorpochainConnection
from corpochain.types.blockchain_format.sized_bytes import bytes32
from corpochain.types.peer_info import PeerInfo
from corpochain.util.errors import Err, ProtocolError
from corpochain.util.ints import uint16
from corpochain.util.network import WebServer, is_in_network, is_localhost
from corpochain.util.ssl_check import verify_ssl_certs_and_keys
...
class CorpochainServer:
    ...
    all_connections: Dict[bytes32, WSCorpochainConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        corpochain_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> CorpochainServer:
        ...
        corpochain_ca_crt_path, corpochain_ca_key_path = corpochain_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                corpochain_ca_crt_path, corpochain_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                corpochain_ca_crt_path, corpochain_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSCorpochainConnection] = []
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Optional[ConnectionCallback] = None) -> None:
        if self.webserver is not None:
            raise RuntimeError("CorpochainServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSCorpochainConnection] = None
        try:
            connection = WSCorpochainConnection.create(
                self._local_type,
                ws,
                self.api,
                self._port,
    ...
    async def connection_added(
        self, connection: WSCorpochainConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSCorpochainConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use corpochain's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.corpochain.pl/") as resp:
                    if resp.status == 200:
