...
from corpochain.protocols.beacon_protocol import RequestPeers, RespondPeers
...
from corpochain.server.address_manager import AddressManager, ExtendedPeerInfo
from corpochain.server.address_manager_store import AddressManagerStore
...
from corpochain.util.ints import uint16, uint64
from corpochain.util.network import IPAddress, get_host_addr
...
class BeaconDiscovery:
    ...
    def __init__(
        ...
        self.server: CorpochainServer = server
    ...
    async def update_peer_timestamp_on_message(self, peer: WSCorpochainConnection) -> None:
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.BEACON
    ...
    async def _introducer_client(self) -> None:
        ...
        async def on_connect(peer: WSCorpochainConnection) -> None:
    ...
    async def on_connect_callback(self, peer: WSCorpochainConnection) -> None:
